...
exec_name = f"test_LB_{case}_DC_{te_s}_v{val_s}_{executions_start}_{executions_end}"
folder_to_save = f"{exec_name}"

dfs_in_full, dfs_out_full = DataLoading.load_data_ext_out(folder, executions, period, sc, None, outp)
...
t_in_fy, t_out_fy, maxs = DataLoading.concat_all_exec_fy(dfs_in_full, dfs_out_full,None,all_executions)
...
dfs_in, dfs_out = DataLoading.return_selection(dfs_dict_list=[dfs_in_full, dfs_out_full],
                                                            indices=indices)
# Convert to pytorch tensors
ts_in, ts_out = DataLoading.split_tr_val_te_ext_out(dfs_in, dfs_out, None, executions, te_s, val_s)

# Concat and normalize
d_ft_in, d_ft_out, maxs_1 = DataLoading.concat_and_normalize_ext_out(ts_in, ts_out, None, executions,normalize= False)
