...
nbc_values = [20,50,100,200]  # Example values for nbc
# min_value = 0.85
# max_value = 1
...
parser.add_argument('--min',    type=float, default=0.85)
...
# Loop through each filter type and nbc
for filter_type in filter_types:
    for nbc in nbc_values:
        print(f"Processing with filter type: {filter_type} and nbc: {nbc}")
        ...
        xx, yy = np.mgrid[min_value:max_value:100j, min_value:max_value:100j]
        ...
        for idx, (f, cm) in enumerate(zip(kde_values_list, clustering_methods)):
            ...
            # Add the reference line y = x for perfect correlation
            ax.plot([min_value, max_value], [min_value, max_value], 'r--', linewidth=1, label='y = x')
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, f"LineUtil_{case}_nc{nbc}_th{min_value}_{max_value}_ft{filter_type}_v2.png"))
