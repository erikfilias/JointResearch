...
def main():
    ...
    df_pint = PINT_approach(args.dir, args.case)

    df_pint.to_csv(_path + '/3.Out' + '/oT_Benefit_Line_PINT_' + args.case + '.csv')

    de_toot = TOOT_approach(args.dir, args.case)

    de_toot.to_csv(_path + '/3.Out' + '/oT_Benefit_Line_TOOT_' + args.case + '.csv')

    df_line_benefit = df_pint*0.5 + de_toot*0.5

    df_line_benefit.to_csv(_path + '/3.Out' + '/oT_Benefit_Line_Total_' + args.case + '.csv')

    # Final time counter for all the code
...
def PINT_approach(DirName, CaseName):
    ...
    df = pd.DataFrame(data=0, columns=pd.MultiIndex.from_tuples(dict_la), index=pd.MultiIndex.from_tuples(dict_psn))
    ...
    for (ni,nf,cc) in dict_lc:
        ...
        # getting the dataset for the specific candidate line
        ...
        for nn in dictSets['nn']:
            ...
            df_eline[np.isnan(df_eline)] = 0
            df_cline[np.isnan(df_cline)] = 0
            ...
            # getting the line benefits
            df.loc[(dictSets['pp'], dictSets['sc'], nn), (ni, nf, cc)] = line_benefit_cline[0] - line_benefit_eline[0]
...
def TOOT_approach(DirName, CaseName):
    ...
    # dataframe with the line benefits generated by the TOOT approach
    df = pd.DataFrame(data=0, columns=pd.MultiIndex.from_tuples(dict_la), index=pd.MultiIndex.from_tuples(dict_psn))

    # generating the dataset for the full (existing and candidate) network
    print('Getting the dataset using all the existing and candidate lines')
    df_Network_full = pd.read_csv(os.path.join(_path, '2.Par','oT_Data_Network_'+CaseName+'.csv'), index_col=[0,1,2])
    df_Network_full = df_Network_full.replace(0.0, float('nan'))
    df_Network_full['FixedInvestmentCost'] = 0
    df_Network_full['FixedChargeRate'    ] = 0
    df_Network_full['BinaryInvestment'   ] = 0
    # df_Network_full = df_Network_full.drop(dict_lc, axis=0)
    print('Full Network indices', len(df_Network_full.index))
    df_Network_full.to_csv(os.path.join(_path, '2.Par','oT_Data_Network_'+CaseName+'.csv'))
    df_full = Input_Dataset_Generator(DirName, CaseName)

    # restoring the original network
    print('Restoring the original network')
    print('Original Network indices', len(df_Network.index))
    df_Network.to_csv(os.path.join(_path, '2.Par','oT_Data_Network_'+CaseName+'.csv'))
    print(f'Time for getting the dataset using existing and candidate lines: {round(time.time() - start_time)} s')
    ...
    for (ni,nf,cc) in dict_lc:
        ...
        df_Network_candidate = df_Network_candidate.drop((ni,nf,cc), axis=0)

        df_Network_candidate['FixedInvestmentCost'] = 0
        df_Network_candidate['FixedChargeRate'    ] = 0
        df_Network_candidate['BinaryInvestment'   ] = 0
        ...
        # getting the line benefits
        for nn in dictSets['nn']:
            df_fline = df_full.loc[nn]
            df_cline = df_line.loc[nn]
            df_fline[np.isnan(df_fline)] = 0
            df_cline[np.isnan(df_cline)] = 0
            x1_fline = torch.from_numpy(df_fline.values)
            x2_cline = torch.from_numpy(df_cline.values)
            InputSizes  = x1_fline.shape[0]
            HiddenSizes = []
            HiddenSizes.extend([int(math.sqrt(InputSizes)), int(math.sqrt(math.sqrt(InputSizes)))])
            OutputSizes = 1
            model    = NN.ObjectiveEstimator_ANN_2hidden_layer(input_size   = InputSizes,
            ...
            model.load_state_dict(torch.load('model_OE_2h_12e_0.01lr_0dor.pth'))
            model.eval()
            y1_fline = model(x1_fline.float())
            y2_cline = model(x2_cline.float())

            # Convert the estimations to NumPy arrays
            line_benefit_fline = y1_fline.detach().numpy()
            line_benefit_cline = y2_cline.detach().numpy()

            # getting the line benefits
            df.loc[(dictSets['pp'], dictSets['sc'], nn), (ni, nf, cc)] = line_benefit_fline[0] - line_benefit_cline[0]
